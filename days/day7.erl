-module(day7).
-export([fst/1, snd/1, permutations/1]).

fst(Input) ->
    Tape = intcode:parse(Input),
    lists:max(concurrent:map(fun(AmpSettings) ->
        lists:foldl(fun(AmpSetting, Output) ->
            {_, IntcodeOutputs} = intcode:run(Tape, [AmpSetting, Output]),
...
Max thruster signal 54321 (from phase setting sequence 0,1,2,3,4):3,23,3,24,1002,24,10,24,1002,23,-1,23,<br/>101,5,23,23,1,24,23,23,4,23,99,0,0
Max thruster signal 65210 (from phase setting sequence 1,0,4,3,2):3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,<br/>1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0
